...
/*!
  ...
  \sa acquire(), key()
 */
QSystemSemaphore::QSystemSemaphore(const std::string &key, int initialValue, AccessMode mode)
 ...
 */
void QSystemSemaphore::setKey(const std::string &key, int initialValue, AccessMode mode)
{
    if (key == d->key && mode == Open)
        return;
    d->clearError();
#if !defined(__WIN32) && !defined(QT_POSIX_IPC)
...
/*!
  ...
  \sa setKey()
 */
std::string QSystemSemaphore::key() const
{
    return d->key;
}
...
/*!
  ...
  \sa error()
 */
std::string QSystemSemaphore::errorString() const
{
